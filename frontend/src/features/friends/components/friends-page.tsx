...
function FriendCard({
  ...
  return (
    <article className={`${featureCardClass} flex min-h-[21rem] flex-col p-6`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-4">
          {friend.avatar ? (
            ...
            <div className="grid h-16 w-16 shrink-0 place-items-center rounded-full border border-white bg-[#22c55e]/80 text-lg font-black text-slate-950 shadow-[0_12px_24px_rgb(34_197_94_/_18%)]">
              {initials}
            </div>
          )}
        ...
        <span className="inline-flex min-h-9 shrink-0 items-center rounded-lg bg-emerald-100 px-3 text-xs font-black text-emerald-800">
          Friend
        </span>
      </div>

      <div className="mt-7 flex flex-col gap-4 text-sm font-bold text-slate-600">
        ...
        {friend.bio ? (
          <p className="min-h-12 leading-6 text-slate-700">&quot;{friend.bio}&quot;</p>
        ) : (
          <p className="min-h-12 leading-6 text-slate-500">Ready for a language exchange.</p>
        )}
      </div>

      <div className="mt-5 flex flex-wrap gap-2">
        <span className="rounded-lg bg-white/80 px-3 py-1.5 text-xs font-black text-slate-700 shadow-sm">
          {friend.nativeLanguage}
        </span>
        <span className="rounded-lg bg-white/80 px-3 py-1.5 text-xs font-black text-slate-700 shadow-sm">
          {friend.targetLanguage}
        </span>
        <span className="rounded-lg bg-red-50 px-3 py-1.5 text-xs font-black text-red-700 shadow-sm">
          {friend.languageLevel}
        </span>
      </div>
...
function FriendsResults({
  ...
  return (
    <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-3" aria-label="Friends list">
      {friends.map((friend) => (
        <FriendCard
          friend={friend}
...
function FriendsCta() {
  return (
    <section className={`${featureCardClass} grid items-center gap-6 p-8 md:grid-cols-[0.8fr_1.2fr]`}>
      <div className="mx-auto grid h-24 w-24 place-items-center rounded-full border-4 border-[#4f46e5] bg-white text-[#4f46e5] shadow-[0_18px_44px_rgb(79_70_229_/_18%)] md:mx-0">
        <Languages aria-hidden="true" size={38} />
      </div>
      <div>
        <h2 className="text-3xl font-black leading-tight text-slate-950">Find more language partners</h2>
        <p className="mt-3 max-w-2xl text-base font-bold leading-7 text-slate-600">
          Keep growing your exchange circle by discovering people with complementary language goals and schedules.
        </p>
        <Link
          className="mt-5 inline-flex min-h-12 items-center justify-center rounded-lg border-2 border-teal-600 px-6 text-sm font-black text-teal-700 transition hover:bg-teal-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-100 motion-reduce:transition-none"
          to="/app/discover"
        >
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f7f6ff] text-slate-950">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-10 sm:px-8">
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <HeroGlassPanel>
            ...
            <p className="mt-4 max-w-3xl text-base font-bold leading-7 text-slate-600">
              Keep track of accepted language partners and jump into chat or video practice when your schedules align.
            </p>
          </HeroGlassPanel>
          ...
          <label className="relative block w-full">
            ...
            <input
              ...
              className="min-h-14 w-full rounded-lg border border-indigo-200 bg-white/78 py-3 pl-12 pr-14 text-base font-bold text-slate-950 outline-none shadow-[0_18px_44px_rgb(79_70_229_/_12%)] backdrop-blur-2xl transition placeholder:text-slate-500 focus:border-[#4f46e5] focus:ring-4 focus:ring-indigo-100 motion-reduce:transition-none"
            ...
            <SlidersHorizontal
              aria-hidden="true"
              className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#4f46e5]"
              size={20}
            />
          </label>
        </section>
        ...
        {removedFriendName ? (
          <p className={`${featureCardClass} p-4 text-sm font-black text-emerald-950`} role="status">
