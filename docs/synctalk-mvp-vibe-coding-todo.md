...
- [ ] 扩展 User 语言资料字段。
- [ ] 实现 `GET/PATCH /api/profile/me`。
- [ ] 校验母语、目标语言、等级、目标、时区。
- [ ] 实现 `/app/profile` 页面。
- [ ] 资料未完善时引导到 profile，保存后进入 discover。
...
- [ ] 返回匹配理由和关系状态。
- [ ] 支持按用户名、语言、简介搜索。
- [ ] 实现 `/app/discover` 页面和用户卡片。
- [ ] 禁用自加、已是好友、已发送请求的重复操作。
...
- [ ] 添加 FriendRequest 和 Friendship 模型。
- [ ] 实现好友请求、请求列表、接受/拒绝、好友列表、移除好友 API。
- [ ] 接受请求时创建一条排序后的好友关系。
- [ ] 实现 `/app/friends` 和 `/app/requests`。
- [ ] 发现页接入发送好友请求。
...
- [ ] 好友请求和请求接受时创建通知。
- [ ] 实现通知列表和标记已读 API。
- [ ] App Shell 中添加通知入口和未读数。
- [ ] metadata 有效时支持跳转；无效时展示无效状态。
